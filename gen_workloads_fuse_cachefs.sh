...
### 用户可修改参数 ###
DEVICE_MNT=/mnt/ZNS
CACHE_MNT=/mnt/cachefs_tt
# MODE_DIR(BASE): zlfs_on_zns | f2fs_on_ssdzns | ext4_on_ssd | cachefs_on_zlfs | cachefs_on_ext4 | cachefs_on_zlfs_medfs
MODE_DIR=cachefs_on_zlfs_medfs
# FILESIZE=65536            # 64KB
FILESIZE=67108864            # 64KB
DIRWIDTH=1
THREADS=32
# IOSIZE=16384              # 16KB
IOSIZE=65536              # 16KB
...
mkdir -p workloads_out/${MODE_DIR}
...
# 获取文件系统当前可用容量（字节）
DISK_SIZE=$(df --output=avail -B1 $DEVICE_MNT | tail -1)
...
for ratio in "${RATIOS[@]}"; do
  ...
  for mode in "${MODES[@]}"; do
    OUTFILE="workloads_out/${MODE_DIR}/${mode}_${ratio}.f"
