...
### 用户可修改参数 ###
CACHE_MNT=/mnt/medfs
# MODE_DIR(BASE): zlfs_on_zns | f2fs_on_ssdzns | ext4_on_ssd | cachefs_on_zlfs | cachefs_on_ext4 | cachefs_on_zlfs_medfs | medfs
MODE_DIR=medfs
# FILESIZE=65536            # 64KB
...
# 获取文件系统当前可用容量（字节）
DISK_SIZE=$(df --output=avail -B1 $CACHE_MNT | tail -1)
